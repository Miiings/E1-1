...
- 원인: 이미 사용 중인 포트에 매핑 시도 - netstat로 포트 사용 확인, docker ps로 다른 컨테이너 확인 및 프로세스 종료
- 해결: 다른 포트로 변경하여 실행 (예: 8081:80)

---

### 문제 2. 맥 위치 바꾸고 git 설정 초기화

- 원인: 새로 git 환경설정 미적용으로 변경사항 push 불가
- 해결: git config로 name, email 설정 후 재개
