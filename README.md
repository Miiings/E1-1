...
- 원인: 이미 사용 중인 포트에 매핑 시도 - 호스트 포트가 이미 사용 중이라면, netstat로 포트 사용 확인, docker ps로 다른 컨테이너 확인, lsof로 프로세스 확인 후 포트 변경 또는 프로세스 종료
...
- 원인: 컨테이너 내부 파일 시스템만 사용 - 컨테이너 삭제 후 데이터가 사라지는 것 방지를 위해 Docker 볼륨을 사용하거나 호스트 디렉토리 바인드 마운트로 방지
